...
def Result_Map(mapping: Callable[[__A], __B], result: FSharpResult_2[__A, __C]) -> FSharpResult_2[__B, __C]:
    if result.tag == 0:
...
def Result_MapError(mapping: Callable[[__A], __B], result: FSharpResult_2[__C, __A]) -> FSharpResult_2[__C, __B]:
    if result.tag == 0:
...
def Result_Bind(binder: Callable[[__A], FSharpResult_2[__B, __C]], result: FSharpResult_2[__A, __C]) -> FSharpResult_2[__B, __C]:
    if result.tag == 0:
...
def Choice_makeChoice1Of2(x: _T1=None) -> FSharpChoice_2[_T1, Any]:
    return FSharpChoice_2(0, x)


def Choice_makeChoice2Of2(x: _T2=None) -> FSharpChoice_2[Any, _T2]:
    return FSharpChoice_2(1, x)


def Choice_tryValueIfChoice1Of2(x: FSharpChoice_2[_T1, Any]) -> Option[_T1]:
    if x.tag == 0:
...
def Choice_tryValueIfChoice2Of2(x: FSharpChoice_2[Any, _T2]) -> Option[_T2]:
    if x.tag == 1:
        return some(x.fields[0])
