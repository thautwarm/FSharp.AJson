...
from typing import (TypeVar, Tuple, Callable, Any, Generic, List)
...
from .option import (Option, some, value as value_1, default_arg)
...
___29732 = TypeVar("___29732")

___29731 = TypeVar("___29731")

___29815 = TypeVar("___29815")

___29814 = TypeVar("___29814")

___29671 = TypeVar("___29671")

___29670 = TypeVar("___29670")

___29697 = TypeVar("___29697")

___29696 = TypeVar("___29696")
...
class FSharpList(Record, Generic[_T]):
    def __init__(self, head: _T, tail: Option[FSharpList[_T]]) -> None:
        super().__init__()
        self.head = head
        self.tail = tail
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        if xs is other:
            ...
            def loop(xs_1_mut: FSharpList[___29732], ys_1_mut: FSharpList[___29732]) -> bool:
                while True:
                    (xs_1, ys_1) = (xs_1_mut, ys_1_mut)
                    match_value : Tuple[Option[FSharpList[___29732]], Option[FSharpList[___29732]]] = (xs_1.tail, ys_1.tail)
                    if match_value[0] is not None:
                        if match_value[1] is not None:
                            xt : FSharpList[___29732] = match_value[0]
                            yt : FSharpList[___29732] = match_value[1]
            ...
            loop : Callable[[FSharpList[___29731], FSharpList[___29731]], bool] = loop
    ...
    def GetHashCode(self) -> int:
        ...
        def loop(i_mut: int, h_mut: int, xs_1_mut: FSharpList[Any]) -> int:
            while True:
                ...
                match_value : Option[FSharpList[_T]] = xs_1.tail
    ...
    def CompareTo(self, other: Any) -> int:
        xs : FSharpList[_T] = self
        def loop(xs_1_mut: FSharpList[___29815], ys_1_mut: FSharpList[___29815]) -> int:
            while True:
                (xs_1, ys_1) = (xs_1_mut, ys_1_mut)
                match_value : Tuple[Option[FSharpList[___29815]], Option[FSharpList[___29815]]] = (xs_1.tail, ys_1.tail)
                if match_value[0] is not None:
                    if match_value[1] is not None:
                        xt : FSharpList[___29815] = match_value[0]
                        yt : FSharpList[___29815] = match_value[1]
...
class ListEnumerator_1(IDisposable, Generic[_T]):
    ...
    def System_Collections_IEnumerator_MoveNext(self) -> bool:
        __ : ListEnumerator_1[_T] = self
        match_value : Option[FSharpList[_T]] = __.it.tail
        if match_value is not None:
            t : FSharpList[_T] = match_value
...
def FSharpList__get_Length(xs: FSharpList[Any]) -> int:
    def loop(i_mut: int, xs_1_mut: FSharpList[Any], xs: FSharpList[_T]=xs) -> int:
        while True:
            (i, xs_1) = (i_mut, xs_1_mut)
            match_value : Option[FSharpList[___29671]] = xs_1.tail
            if match_value is not None:
                i_mut = i + 1
...
def FSharpList__get_Head(xs: FSharpList[_T]) -> _T:
    match_value : Option[FSharpList[_T]] = xs.tail
    if match_value is not None:
        return xs.head
...
def FSharpList__get_Item_Z524259A4(xs: FSharpList[_T], index: int) -> _T:
    def loop(i_mut: int, xs_1_mut: FSharpList[___29697], xs: FSharpList[_T]=xs, index: int=index) -> ___29697:
        while True:
            (i, xs_1) = (i_mut, xs_1_mut)
            match_value : Option[FSharpList[___29697]] = xs_1.tail
            if match_value is not None:
                if i == index:
                    return xs_1.head
...
def singleton(x: __A=None) -> FSharpList[__A]:
...
def try_head(xs: FSharpList[_T]) -> Option[_T]:
...
def try_last(xs_mut: FSharpList[_T]) -> Option[_T]:
    while True:
        (xs,) = (xs_mut,)
        if FSharpList__get_IsEmpty(xs):
            return None
...
def to_array(xs: FSharpList[_T]) -> List[_T]:
    ...
    def loop(i_mut: int, xs_1_mut: FSharpList[Any], xs: FSharpList[_T]=xs) -> None:
        while True:
            (i, xs_1) = (i_mut, xs_1_mut)
...
def reverse(xs: FSharpList[_T]) -> FSharpList[_T]:
    def arrow_73(acc: FSharpList[_T], x: _T=None, xs: FSharpList[_T]=xs) -> FSharpList[_T]:
        return FSharpList_Cons_305B8EAC(x, acc)
    
    return fold(arrow_73, FSharpList_get_Empty(), xs)


def fold_back(folder: Callable[[_T, _STATE], _STATE], xs: FSharpList[_T], state: _STATE=None) -> _STATE:
...
def fold_indexed(folder: Callable[[int, _STATE, _T], _STATE], state: _STATE, xs: FSharpList[_T]) -> _STATE:
    def loop(i_mut: int, acc_mut: _STATE, xs_1_mut: FSharpList[Any], folder: Callable[[int, _STATE, _T], _STATE]=folder, state: _STATE=state, xs: FSharpList[_T]=xs) -> _STATE:
        while True:
...
def fold_back2(folder: Callable[[_T1, _T2, _STATE], _STATE], xs: FSharpList[_T1], ys: FSharpList[_T2], state: _STATE=None) -> _STATE:
    return fold_back2_1(folder, to_array(xs), to_array(ys), state)


def unfold(gen: Callable[[_STATE], Option[Tuple[_T, _STATE]]], state: _STATE=None) -> FSharpList[_T]:
    def loop(acc_mut: Any, node_mut: FSharpList[_T], gen: Callable[[_STATE], Option[Tuple[_T, _STATE]]]=gen, state: _STATE=state) -> FSharpList[_T]:
        while True:
            (acc, node) = (acc_mut, node_mut)
            match_value : Option[Tuple[_T, _STATE]] = gen(acc)
            if match_value is not None:
                acc_mut = match_value[1]
                def arrow_74(acc: _STATE=acc, node: FSharpList[_T]=node) -> FSharpList[Any]:
...
def iterate(action: Callable[[__A], None], xs: FSharpList[__A]) -> None:
    def arrow_75(unit_var0: None, x: Any=None, action: Callable[[__A], None]=action, xs: FSharpList[__A]=xs) -> None:
        action(x)
...
def iterate2(action: Callable[[__A, __B], None], xs: FSharpList[__A], ys: FSharpList[__B]) -> None:
    def arrow_76(unit_var0: None, x: Any, y: Any=None, action: Callable[[__A, __B], None]=action, xs: FSharpList[__A]=xs, ys: FSharpList[__B]=ys) -> None:
        action(x, y)
...
def iterate_indexed(action: Callable[[int, __A], None], xs: FSharpList[__A]) -> None:
    def arrow_77(i: int, x: Any=None, action: Callable[[int, __A], None]=action, xs: FSharpList[__A]=xs) -> int:
        action(i, x)
        return i + 1
...
def iterate_indexed2(action: Callable[[int, __A, __B], None], xs: FSharpList[__A], ys: FSharpList[__B]) -> None:
    def arrow_78(i: int, x: Any, y: Any=None, action: Callable[[int, __A, __B], None]=action, xs: FSharpList[__A]=xs, ys: FSharpList[__B]=ys) -> int:
        action(i, x, y)
        return i + 1
    ...
    if is_array_like(xs):
        ...
        with get_enumerator(xs) as enumerator:
            while enumerator.System_Collections_IEnumerator_MoveNext():
                ...
                def arrow_79(xs: IEnumerable[_T]=xs) -> FSharpList[Any]:
...
def concat(lists: IEnumerable[FSharpList[_T]]) -> FSharpList[_T]:
    root : FSharpList[_T] = FSharpList_get_Empty()
    node : FSharpList[_T] = root
    def action(xs: FSharpList[Any], lists: IEnumerable[FSharpList[_T]]=lists) -> None:
        nonlocal node
        def arrow_80(acc: FSharpList[_T], x: _T=None, xs: FSharpList[_T]=xs) -> FSharpList[_T]:
            t : FSharpList[_T] = FSharpList(x, None)
            acc.tail = t
            return t
...
def scan(folder: Callable[[_STATE, _T], _STATE], state: _STATE, xs: FSharpList[_T]) -> FSharpList[_STATE]:
    ...
    def arrow_81(folder: Callable[[_STATE, _T], _STATE]=folder, state: _STATE=state, xs: FSharpList[_T]=xs) -> FSharpList[Any]:
    ...
    while not FSharpList__get_IsEmpty(xs_3):
        ...
        def arrow_83(folder: Callable[[_STATE, _T], _STATE]=folder, state: _STATE=state, xs: FSharpList[_T]=xs) -> FSharpList[Any]:
            xs_4 : FSharpList[_STATE] = node
            def arrow_82(_unit: Any=None) -> FSharpList[Any]:
...
def scan_back(folder: Callable[[_T, _STATE], _STATE], xs: FSharpList[_T], state: _STATE=None) -> FSharpList[_STATE]:
    return of_array(scan_back_1(folder, to_array(xs), state, None))


def append(xs: FSharpList[_T], ys: FSharpList[_T]) -> FSharpList[_T]:
    def arrow_84(acc: FSharpList[_T], x: _T=None, xs: FSharpList[_T]=xs, ys: FSharpList[_T]=ys) -> FSharpList[_T]:
        return FSharpList_Cons_305B8EAC(x, acc)
...
def collect(mapping: Callable[[_T], FSharpList[_U]], xs: FSharpList[_T]) -> FSharpList[_U]:
    ...
    while not FSharpList__get_IsEmpty(ys):
        ...
        while not FSharpList__get_IsEmpty(zs):
            def arrow_86(mapping: Callable[[_T], FSharpList[_U]]=mapping, xs: FSharpList[_T]=xs) -> FSharpList[Any]:
                xs_1 : FSharpList[_U] = node
                def arrow_85(_unit: Any=None) -> FSharpList[Any]:
...
def map_indexed(mapping: Callable[[int, _T], _U], xs: FSharpList[_T]) -> FSharpList[_U]:
    root : FSharpList[_U] = FSharpList_get_Empty()
    def folder(i: int, acc: FSharpList[_U], x: Any=None, mapping: Callable[[int, _T], _U]=mapping, xs: FSharpList[_T]=xs) -> FSharpList[_U]:
        def arrow_87(i: int=i, acc: FSharpList[_U]=acc, x: _T=x) -> FSharpList[Any]:
...
def map(mapping: Callable[[_T], _U], xs: FSharpList[_T]) -> FSharpList[_U]:
    root : FSharpList[_U] = FSharpList_get_Empty()
    def folder(acc: FSharpList[_U], x: Any=None, mapping: Callable[[_T], _U]=mapping, xs: FSharpList[_T]=xs) -> FSharpList[_U]:
        def arrow_88(acc: FSharpList[_U]=acc, x: _T=x) -> FSharpList[Any]:
...
def indexed(xs: FSharpList[__A]) -> FSharpList[Tuple[int, __A]]:
    def arrow_89(i: int, x: __A=None, xs: FSharpList[__A]=xs) -> Tuple[int, __A]:
        return (i, x)
...
def map2(mapping: Callable[[_T1, _T2], _U], xs: FSharpList[_T1], ys: FSharpList[_T2]) -> FSharpList[_U]:
    root : FSharpList[_U] = FSharpList_get_Empty()
    def folder(acc: FSharpList[_U], x: Any, y: Any=None, mapping: Callable[[_T1, _T2], _U]=mapping, xs: FSharpList[_T1]=xs, ys: FSharpList[_T2]=ys) -> FSharpList[_U]:
        def arrow_90(acc: FSharpList[_U]=acc, x: _T1=x, y: _T2=y) -> FSharpList[Any]:
...
def map_indexed2(mapping: Callable[[int, _T1, _T2], _U], xs: FSharpList[_T1], ys: FSharpList[_T2]) -> FSharpList[_U]:
    def loop(i_mut: int, acc_mut: FSharpList[_U], xs_1_mut: FSharpList[Any], ys_1_mut: FSharpList[Any], mapping: Callable[[int, _T1, _T2], _U]=mapping, xs: FSharpList[_T1]=xs, ys: FSharpList[_T2]=ys) -> FSharpList[_U]:
        while True:
            ...
            if True if (FSharpList__get_IsEmpty(xs_1)) else (FSharpList__get_IsEmpty(ys_1)):
                ...
                i_mut = i + 1
                def arrow_91(i: int=i, acc: FSharpList[_U]=acc, xs_1: FSharpList[_T1]=xs_1, ys_1: FSharpList[_T2]=ys_1) -> FSharpList[Any]:
...
def map3(mapping: Callable[[_T1, _T2, _T3], _U], xs: FSharpList[_T1], ys: FSharpList[_T2], zs: FSharpList[_T3]) -> FSharpList[_U]:
    def loop(acc_mut: FSharpList[_U], xs_1_mut: FSharpList[Any], ys_1_mut: FSharpList[Any], zs_1_mut: FSharpList[Any], mapping: Callable[[_T1, _T2, _T3], _U]=mapping, xs: FSharpList[_T1]=xs, ys: FSharpList[_T2]=ys, zs: FSharpList[_T3]=zs) -> FSharpList[_U]:
        while True:
            ...
            if True if (True if (FSharpList__get_IsEmpty(xs_1)) else (FSharpList__get_IsEmpty(ys_1))) else (FSharpList__get_IsEmpty(zs_1)):
                ...
                def arrow_92(acc: FSharpList[_U]=acc, xs_1: FSharpList[_T1]=xs_1, ys_1: FSharpList[_T2]=ys_1, zs_1: FSharpList[_T3]=zs_1) -> FSharpList[Any]:
...
def map_fold(mapping: Callable[[_STATE, _T], Tuple[_RESULT, _STATE]], state: _STATE, xs: FSharpList[_T]) -> Tuple[FSharpList[_RESULT], _STATE]:
    ...
    def folder(tupled_arg: Tuple[FSharpList[_RESULT], _STATE], x: Any=None, mapping: Callable[[_STATE, _T], Tuple[_RESULT, _STATE]]=mapping, state: _STATE=state, xs: FSharpList[_T]=xs) -> Tuple[FSharpList[_RESULT], _STATE]:
        pattern_input : Tuple[_RESULT, _STATE] = mapping(tupled_arg[1], x)
        def arrow_93(tupled_arg: Tuple[FSharpList[_RESULT], _STATE]=tupled_arg, x: _T=x) -> FSharpList[Any]:
...
def map_fold_back(mapping: Callable[[_T, _STATE], Tuple[_RESULT, _STATE]], xs: FSharpList[_T], state: _STATE=None) -> Tuple[FSharpList[_RESULT], _STATE]:
    def arrow_94(acc: _STATE, x: Any=None, mapping: Callable[[_T, _STATE], Tuple[_RESULT, _STATE]]=mapping, xs: FSharpList[_T]=xs, state: _STATE=state) -> Tuple[_RESULT, _STATE]:
        return mapping(x, acc)
    
    return map_fold(arrow_94, state, reverse(xs))


def try_pick(f: Callable[[_T], Option[__A]], xs: FSharpList[_T]) -> Option[__A]:
    def loop(xs_1_mut: FSharpList[Any], f: Callable[[_T], Option[__A]]=f, xs: FSharpList[_T]=xs) -> Option[Any]:
        while True:
            (xs_1,) = (xs_1_mut,)
            if FSharpList__get_IsEmpty(xs_1):
                return None
                ...
                match_value : Option[__A] = f(FSharpList__get_Head(xs_1))
                if match_value is None:
                    xs_1_mut = FSharpList__get_Tail(xs_1)
                    continue
...
def pick(f: Callable[[__A], Option[__B]], xs: FSharpList[__A]) -> __B:
    match_value : Option[__B] = try_pick(f, xs)
    if match_value is None:
...
def try_find(f: Callable[[__A], bool], xs: FSharpList[__A]) -> Option[__A]:
    def arrow_95(x: __A=None, f: Callable[[__A], bool]=f, xs: FSharpList[__A]=xs) -> Option[__A]:
...
def find(f: Callable[[__A], bool], xs: FSharpList[__A]) -> __A:
    match_value : Option[__A] = try_find(f, xs)
    if match_value is None:
        raise Exception(SR_keyNotFoundAlt)
...
def try_find_back(f: Callable[[__A], bool], xs: FSharpList[__A]) -> Option[__A]:
    return try_find_back_1(f, to_array(xs))


def find_back(f: Callable[[__A], bool], xs: FSharpList[__A]) -> __A:
    match_value : Option[__A] = try_find_back(f, xs)
    if match_value is None:
        raise Exception(SR_keyNotFoundAlt)
...
def try_find_index(f: Callable[[_T], bool], xs: FSharpList[_T]) -> Option[int]:
    def loop(i_mut: int, xs_1_mut: FSharpList[Any], f: Callable[[_T], bool]=f, xs: FSharpList[_T]=xs) -> Option[int]:
        while True:
            (i, xs_1) = (i_mut, xs_1_mut)
            if FSharpList__get_IsEmpty(xs_1):
                return None
...
def find_index(f: Callable[[__A], bool], xs: FSharpList[__A]) -> int:
    match_value : Option[int] = try_find_index(f, xs)
    if match_value is None:
        raise Exception(SR_keyNotFoundAlt)
...
def try_find_index_back(f: Callable[[__A], bool], xs: FSharpList[__A]) -> Option[int]:
    return try_find_index_back_1(f, to_array(xs))


def find_index_back(f: Callable[[__A], bool], xs: FSharpList[__A]) -> int:
    match_value : Option[int] = try_find_index_back(f, xs)
    if match_value is None:
        raise Exception(SR_keyNotFoundAlt)
...
def try_item(n: int, xs: FSharpList[_T]) -> Option[_T]:
    def loop(i_mut: int, xs_1_mut: FSharpList[_T], n: int=n, xs: FSharpList[_T]=xs) -> Option[_T]:
        while True:
            (i, xs_1) = (i_mut, xs_1_mut)
            if FSharpList__get_IsEmpty(xs_1):
                return None
...
def filter(f: Callable[[_T], bool], xs: FSharpList[_T]) -> FSharpList[_T]:
    root : FSharpList[_T] = FSharpList_get_Empty()
    def folder(acc: FSharpList[_T], x: _T=None, f: Callable[[_T], bool]=f, xs: FSharpList[_T]=xs) -> FSharpList[_T]:
        if f(x):
            t : FSharpList[_T] = FSharpList(x, None)
            acc.tail = t
            return t
...
def partition(f: Callable[[_T], bool], xs: FSharpList[_T]) -> Tuple[FSharpList[_T], FSharpList[_T]]:
    ...
    def folder(tupled_arg: Tuple[FSharpList[_T], FSharpList[_T]], f: Callable[[_T], bool]=f, xs: FSharpList[_T]=xs) -> Callable[[_T], Tuple[FSharpList[_T], FSharpList[_T]]]:
        ...
        def arrow_98(x: _T=None, tupled_arg: Tuple[FSharpList[_T], FSharpList[_T]]=tupled_arg) -> Tuple[FSharpList[_T], FSharpList[_T]]:
            def arrow_96(_unit: Any=None) -> FSharpList[Any]:
                t : FSharpList[_T] = FSharpList(x, None)
                lacc.tail = t
                return t
            
            def arrow_97(_unit: Any=None) -> FSharpList[Any]:
...
def choose(f: Callable[[_T], Option[_T]], xs: FSharpList[_T]) -> FSharpList[_T]:
    root : FSharpList[_T] = FSharpList_get_Empty()
    def folder(acc: FSharpList[_T], x: _T=None, f: Callable[[_T], Option[_T]]=f, xs: FSharpList[_T]=xs) -> FSharpList[_T]:
        match_value : Option[_T] = f(x)
        if match_value is None:
            return acc
...
def contains(value: _T, xs: FSharpList[_T], eq: IEqualityComparer[Any]) -> bool:
    def arrow_99(v: Any=None, value: _T=value, xs: FSharpList[_T]=xs, eq: IEqualityComparer[Any]=eq) -> bool:
...
def initialize(n: int, f: Callable[[int], _T]) -> FSharpList[_T]:
    ...
    for i in range(0, (n - 1) + 1, 1):
        def arrow_100(n: int=n, f: Callable[[int], _T]=f) -> FSharpList[Any]:
...
def replicate(n: int, x: __A=None) -> FSharpList[__A]:
    def arrow_101(_arg1: int, n: int=n, x: __A=x) -> Any:
        return x
...
def for_all(f: Callable[[__A], bool], xs: FSharpList[__A]) -> bool:
    def arrow_102(acc: bool, x: Any=None, f: Callable[[__A], bool]=f, xs: FSharpList[__A]=xs) -> bool:
...
def for_all2(f: Callable[[__A, __B], bool], xs: FSharpList[__A], ys: FSharpList[__B]) -> bool:
    def arrow_103(acc: bool, x: Any, y: Any=None, f: Callable[[__A, __B], bool]=f, xs: FSharpList[__A]=xs, ys: FSharpList[__B]=ys) -> bool:
...
def unzip(xs: FSharpList[Tuple[__A, __B]]) -> Tuple[FSharpList[__A], FSharpList[__B]]:
    def arrow_104(tupled_arg: Tuple[__A, __B], tupled_arg_1: Tuple[FSharpList[__A], FSharpList[__B]], xs: FSharpList[Tuple[__A, __B]]=xs) -> Tuple[FSharpList[__A], FSharpList[__B]]:
...
def unzip3(xs: FSharpList[Tuple[__A, __B, __C]]) -> Tuple[FSharpList[__A], FSharpList[__B], FSharpList[__C]]:
    def arrow_105(tupled_arg: Tuple[__A, __B, __C], tupled_arg_1: Tuple[FSharpList[__A], FSharpList[__B], FSharpList[__C]], xs: FSharpList[Tuple[__A, __B, __C]]=xs) -> Tuple[FSharpList[__A], FSharpList[__B], FSharpList[__C]]:
...
def zip(xs: FSharpList[__A], ys: FSharpList[__B]) -> FSharpList[Tuple[__A, __B]]:
    def arrow_106(x: __A, y: __B=None, xs: FSharpList[__A]=xs, ys: FSharpList[__B]=ys) -> Tuple[__A, __B]:
        return (x, y)
...
def zip3(xs: FSharpList[__A], ys: FSharpList[__B], zs: FSharpList[__C]) -> FSharpList[Tuple[__A, __B, __C]]:
    def arrow_107(x: __A, y: __B, z: __C=None, xs: FSharpList[__A]=xs, ys: FSharpList[__B]=ys, zs: FSharpList[__C]=zs) -> Tuple[__A, __B, __C]:
        return (x, y, z)
...
def sort(xs: FSharpList[_T], comparer: IComparer[_T]) -> FSharpList[_T]:
    def arrow_108(x: _T, y: _T=None, xs: FSharpList[_T]=xs, comparer: IComparer[_T]=comparer) -> int:
...
def sort_by(projection: Callable[[_T], _U], xs: FSharpList[_T], comparer: IComparer[_U]) -> FSharpList[_T]:
    def arrow_109(x: _T, y: _T=None, projection: Callable[[_T], _U]=projection, xs: FSharpList[_T]=xs, comparer: IComparer[_U]=comparer) -> int:
...
def sort_descending(xs: FSharpList[_T], comparer: IComparer[_T]) -> FSharpList[_T]:
    def arrow_112(x: _T, y: _T=None, xs: FSharpList[_T]=xs, comparer: IComparer[_T]=comparer) -> int:
        return comparer.Compare(x, y) * -1
    
    return sort_with(arrow_112, xs)


def sort_by_descending(projection: Callable[[_T], _U], xs: FSharpList[_T], comparer: IComparer[_U]) -> FSharpList[_T]:
    def arrow_114(x: _T, y: _T=None, projection: Callable[[_T], _U]=projection, xs: FSharpList[_T]=xs, comparer: IComparer[_U]=comparer) -> int:
        return comparer.Compare(projection(x), projection(y)) * -1
    
    return sort_with(arrow_114, xs)


def sum(xs: FSharpList[_T], adder: IGenericAdder_1[_T]) -> _T:
    def arrow_119(acc: _T, x: _T=None, xs: FSharpList[_T]=xs, adder: IGenericAdder_1[_T]=adder) -> _T:
        return adder.Add(acc, x)
    
    return fold(arrow_119, adder.GetZero(), xs)


def sum_by(f: Callable[[_T], _U], xs: FSharpList[_T], adder: IGenericAdder_1[_U]) -> _U:
    def arrow_124(acc: _U, x: Any=None, f: Callable[[_T], _U]=f, xs: FSharpList[_T]=xs, adder: IGenericAdder_1[_U]=adder) -> _U:
        return adder.Add(acc, f(x))
    
    return fold(arrow_124, adder.GetZero(), xs)


def max_by(projection: Callable[[_T], _U], xs: FSharpList[_T], comparer: IComparer[_U]) -> _T:
    def arrow_125(x: _T, y: _T=None, projection: Callable[[_T], _U]=projection, xs: FSharpList[_T]=xs, comparer: IComparer[_U]=comparer) -> _T:
...
def max(xs: FSharpList[_T], comparer: IComparer[_T]) -> _T:
    def arrow_126(x: _T, y: _T=None, xs: FSharpList[_T]=xs, comparer: IComparer[_T]=comparer) -> _T:
...
def min_by(projection: Callable[[_T], _U], xs: FSharpList[_T], comparer: IComparer[_U]) -> _T:
    def arrow_127(x: _T, y: _T=None, projection: Callable[[_T], _U]=projection, xs: FSharpList[_T]=xs, comparer: IComparer[_U]=comparer) -> _T:
...
def min(xs: FSharpList[_T], comparer: IComparer[_T]) -> _T:
    def arrow_128(x: _T, y: _T=None, xs: FSharpList[_T]=xs, comparer: IComparer[_T]=comparer) -> _T:
        return x if (comparer.Compare(y, x) > 0) else (y)
    
    return reduce(arrow_128, xs)


def average(xs: FSharpList[_T], averager: IGenericAverager_1[_T]) -> _T:
    count : int = 0
    def folder(acc: _T, x: _T=None, xs: FSharpList[_T]=xs, averager: IGenericAverager_1[_T]=averager) -> _T:
        nonlocal count
        count = (count + 1) or 0
...
def average_by(f: Callable[[_T], _U], xs: FSharpList[_T], averager: IGenericAverager_1[_U]) -> _U:
    count : int = 0
    def arrow_129(acc: _U, x: Any=None, f: Callable[[_T], _U]=f, xs: FSharpList[_T]=xs, averager: IGenericAverager_1[_U]=averager) -> _U:
        nonlocal count
        count = (count + 1) or 0
        return averager.Add(acc, f(x))
    
    total : _U = fold(arrow_129, averager.GetZero(), xs)
    return averager.DivideByInt(total, count)
...
def all_pairs(xs: FSharpList[_T1], ys: FSharpList[_T2]) -> FSharpList[Tuple[_T1, _T2]]:
    ...
    def arrow_133(x: Any=None, xs: FSharpList[_T1]=xs, ys: FSharpList[_T2]=ys) -> None:
        def arrow_132(y: Any=None) -> None:
            nonlocal node
            def arrow_131(_unit: Any=None) -> FSharpList[Tuple[_T1, _T2]]:
                xs_1 : FSharpList[Tuple[_T1, _T2]] = node
                def arrow_130(_unit: Any=None) -> FSharpList[Any]:
        ...
        iterate(arrow_132, ys)
    
    iterate(arrow_133, xs)
    ...
    def loop(i_mut: int, acc_mut: FSharpList[_T], xs_1_mut: FSharpList[_T], count: int=count, xs: FSharpList[_T]=xs) -> FSharpList[_T]:
        while True:
            ...
            if i <= 0:
                ...
                i_mut = i - 1
                def arrow_142(i: int=i, acc: FSharpList[_T]=acc, xs_1: FSharpList[_T]=xs_1) -> FSharpList[Any]:
    ...
    def loop(acc_mut: FSharpList[_T], xs_1_mut: FSharpList[_T], predicate: Callable[[_T], bool]=predicate, xs: FSharpList[_T]=xs) -> FSharpList[_T]:
        while True:
            ...
            if FSharpList__get_IsEmpty(xs_1):
                ...
                def arrow_143(acc: FSharpList[_T]=acc, xs_1: FSharpList[_T]=xs_1) -> FSharpList[Any]:
                    t : FSharpList[_T] = FSharpList(FSharpList__get_Head(xs_1), None)
                    acc.tail = t
                    return t
    ...
    def loop(i_mut: int, acc_mut: FSharpList[_T], xs_1_mut: FSharpList[_T], count: int=count, xs: FSharpList[_T]=xs) -> FSharpList[_T]:
        while True:
            ...
            if i <= 0:
                ...
                i_mut = i - 1
                def arrow_144(i: int=i, acc: FSharpList[_T]=acc, xs_1: FSharpList[_T]=xs_1) -> FSharpList[Any]:
                    t : FSharpList[_T] = FSharpList(FSharpList__get_Head(xs_1), None)
                    acc.tail = t
                    return t
...
def get_slice(start_index: Option[int], end_index: Option[int], xs: FSharpList[_T]) -> FSharpList[_T]:
...
def try_exactly_one(xs: FSharpList[_T]) -> Option[_T]:
...
def insert_at(index: int, y: _T, xs: FSharpList[_T]) -> FSharpList[_T]:
    i : int = -1
    is_done : bool = False
    def folder(acc: FSharpList[_T], x: _T=None, index: int=index, y: _T=y, xs: FSharpList[_T]=xs) -> FSharpList[_T]:
        nonlocal i, is_done
        i = (i + 1) or 0
        if i == index:
            is_done = True
    ...
    def arrow_145(index: int=index, y: _T=y, xs: FSharpList[_T]=xs) -> FSharpList[Any]:
        raise Exception((SR_indexOutOfBounds + "\\nParameter name: ") + "index")
    
    return reverse(result if (is_done) else (FSharpList_Cons_305B8EAC(y, result) if ((i + 1) == index) else (arrow_145())))
...
def insert_many_at(index: int, ys: IEnumerable[_T], xs: FSharpList[_T]) -> FSharpList[_T]:
    ...
    def folder(acc: FSharpList[_T], x: _T=None, index: int=index, ys: IEnumerable[_T]=ys, xs: FSharpList[_T]=xs) -> FSharpList[_T]:
        nonlocal i, is_done
        i = (i + 1) or 0
        if i == index:
            is_done = True
    ...
    result : FSharpList[_T] = fold(folder, FSharpList_get_Empty(), xs)
    def arrow_146(index: int=index, ys: IEnumerable[_T]=ys, xs: FSharpList[_T]=xs) -> FSharpList[Any]:
        raise Exception((SR_indexOutOfBounds + "\\nParameter name: ") + "index")
    
    return reverse(result if (is_done) else (append(ys_1, result) if ((i + 1) == index) else (arrow_146())))


def remove_at(index: int, xs: FSharpList[_T]) -> FSharpList[_T]:
    i : int = -1
    is_done : bool = False
    def f(_arg1: Any=None, index: int=index, xs: FSharpList[_T]=xs) -> bool:
        nonlocal i, is_done
        i = (i + 1) or 0
        if i == index:
            is_done = True
...
def remove_many_at(index: int, count: int, xs: FSharpList[_T]) -> FSharpList[_T]:
    i : int = -1
    status : int = -1
    def f(_arg1: Any=None, index: int=index, count: int=count, xs: FSharpList[_T]=xs) -> bool:
        nonlocal i, status
        i = (i + 1) or 0
        if i == index:
            status = 0
...
def update_at(index: int, y: _T, xs: FSharpList[_T]) -> FSharpList[_T]:
    is_done : bool = False
    def mapping(i: int, x: _T=None, index: int=index, y: _T=y, xs: FSharpList[_T]=xs) -> _T:
        nonlocal is_done
        if i == index:
            is_done = True
            return y
